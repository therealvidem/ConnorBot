...
const loggingChannels = new Keyv('sqlite://data.db', {
    namespace: 'loggingChannels'
});
const emojisSaved = new Keyv('sqlite://data.db', {
    namespace: 'loggedEmojis'
});
...
async function initializeDatabase(guild, channelId, channelName) {
    ...
    makeDir(`${dir}/${guildId}`);
    const database = new Keyv(`${logDbDir}/${guildId}/log.db`, {
        namespace: channelId
    });
        ...
        loggingChannels.get(channel.id).then((logging) => {
            if (logging) {
                ...
                if (!fs.existsSync(`${guildDir}/log.db`)) {
                    ...
                    loggingGuilds[guildId][channel.id] = new Keyv(`${logDbDir}/${guildId}/log.db`, {
